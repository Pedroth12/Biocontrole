...
server <- function(input, output, session) {
  ...
  observe({
    if (is.null(basidio_cache())) {
      ...
      # Buscar dados para todos os tipos de qualidade
      data_research <- tryCatch({
        rinat::get_inat_obs(
        ...
        message("Erro ao buscar dados research: ", e$message)
        return(data.frame())
      })
      
      data_casual <- tryCatch({
        rinat::get_inat_obs(
          taxon_name = "Basidiomycota",
          quality = "casual",
          bounds = brazil_bounds,
        ...
        message("Erro ao buscar dados casual: ", e$message)
        return(data.frame())
      })
      
      data_any <- tryCatch({
        if (is.null(data_research) || nrow(data_research) == 0) {
          data.frame()
        } else {
          data_research
        }
      ...
      # Combinar os dados
      combined_data <- bind_rows(data_research, data_casual)
      ...
      processed_data <- combined_data %>%
        ...
        mutate(
          ...
          quality_grade = ifelse(is.na(quality_grade), "casual", quality_grade)  # Padrão para casual se não tiver informação
        ) %>%
  ...
  observeEvent(input$reset_filters, {
    ...
    updateSelectInput(session, "quality", selected = "research")
  })
  ...
  filtered_data <- reactive({
    ...
    if (!is.null(input$species) && input$species != "") {
      filtered <- filter(filtered, scientific_name == input$species)
    }
    if (!is.null(input$year_range)) {
      filtered <- filter(filtered, between(observed_year, input$year_range[1], input$year_range[2]))
    }
    if (!is.null(input$state) && input$state != "") {
      filtered <- filter(filtered, state == input$state)
    }
    if (!is.null(input$quality) && input$quality != "any") {
      filtered <- filter(filtered, quality_grade == input$quality)
    }
    ...
    leaflet(data) %>%
      ...
      addCircleMarkers(
        ...
        popup = ~paste0(
          ...
          "<b>Local:</b> ", place_guess, "<br>",
          "<b>Qualidade:</b> ", quality_grade, "<br><br>",
